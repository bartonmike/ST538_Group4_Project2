...
First, we tried fitting a default random forest classifier to the training data and evaluating its performance on both the training and validation datasets. Due to the strong class imbalance in the loan default variable, the initial models showed poor sensitivity for detecting loan defaults, despite relatively high overall accuracy. 
...
#| label: randomforest-model2-pred-validation
 ...
 #   # 1  3056    78
```
Upon further exploration of the data, we found that random forest was having issues based on the  Gini importance metric, as it favors high-cardinality variables such as CURRENT_PINCODE_ID, DATE_OF_BIRTH, DISBURSED_AMOUNT because they offer more split opportunities, not necessarily because they better predict defaults. Meanwhile, credit-risk variables that should be more informative such as PERFORM_CNS_SCORE, delinquency measures, leverage ratio, and our key predictors rank in the middle, likely due to class imbalance rather than a lack of predictive value. Before moving forward, we switched the importance metric to use Mean decrease accuracy


```{r}
#| label: randomforest-old-model2
# #| echo: FALSE
# #| include: FALSE
#  rf.sim2 <- randomForest(
...
```{r}
#| label: randomforest-old-model2-prediction
# #| echo: FALSE
# #| include: FALSE
# # 
...
```{r}
#| label: randomforest-model-rftune
#| echo: FALSE
#| include: FALSE
...
```{r}
#| label: randomforest-model-rftune-prediction
#| echo: FALSE
#| include: FALSE
...
The next model we trained on the cleaned dataset was utilizing an XGBoost classifier. We utilized the scale_pos_weight parameter when training the model to try compensating for the imbalance in the response variable. After training the model still had low accuracies, with a tested AUC score of 0.5274. We then explored different thresholds and found that 0.10 and 0.15 give better results for the imbalanced data. 

It may be necessary to utilize LASSO or other methods to try and remove overlapping variables before training the XGBoost models.
^^^^ Did we ever use lasso in this? did we ever explore further solutions on XGBoost it seems like we've really under explored that method compared to random forest

## Comparison of Predicted Results

```{r}
#| label: all-models-pred-test-data
...
Above are the results for our improved Randomforest and XGBoost. As we can see the tuned methods still issues with accuracy and imbalances. 
...
Another issue we had was in the random forest prioritization of variables. Basedo on our assessement of variable importance, there were certain markers that should've been utilized as the most important. However, we determined that random forest was prioritizing variables with high cardinality. We exerpimented with switching the variable importance evaluation method which proved useful.
