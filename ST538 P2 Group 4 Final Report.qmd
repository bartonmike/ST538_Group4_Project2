...
```{r}
#| label: eda-feature-selection3-stress-score-table
#| echo: FALSE
...
```{r}
#| label: eda-feature-selection4-stress-score-graph
#| echo: FALSE
...
```{r}
#| label: data-spltting-thin-normal
#| echo: FALSE
#| include: FALSE
...
Prior to model fitting, several columns were removed from the feature set. Administrative identifiers with no predictive value (e.g., UNIQUEID, BRANCH\_ID, EMPLOYEE_CODE\_ID, and EMPLOYEE\_CODE\_ID) were excluded, as were variables whose information had already been incorporated into engineered features. In addition, columns containing substantial missingness were removed. These missing values were either structural (e.g., thin_file_reason is only populated for thin-file borrowers) or resulted from calculations involving a zero PRIMARY\_INSTA\L_AMT (e.g., new\_loan\_emi\_ratio). Because the relevant information was captured elsewhere in the feature set, these variables were excluded from the modeling dataset.

Finally, the four weakest KYC flags were dropped in favor of the summary measures kyc\_doc_count and strong\_kyc_flag, and the secondary account financial detail columns were removed as redundant given the derived exposure flags already in the feature set. LOAN\_DEFAULT was excluded as the response variable.

## Predictive Model Fitting

```{r}
#| label: data-split-train-validate-test
#| echo: FALSE
#| include: FALSE
...
```{r}
#| label: randomforest-data-cleaning
#| echo: FALSE
#| include: FALSE
...
```{r}
#| label: randomforest-model1-pred-validation
#| echo: FALSE
#| include: FALSE
#| eval: FALSE
...
First, we tried fitting a default random forest classifier to the training data and evaluating its performance on both the training and validation datasets. Due to the strong class imbalance in the loan default variable, the initial models showed poor sensitivity for detecting loan defaults, despite relatively high overall accuracy. The majority of the mis-classifications occurred on loan defaults.



```{r}
#| label: randomforest-model1-diagnostics
#| eval: FALSE
#| echo: FALSE
#| include: FALSE
#| fig.height: 8
...
```{r}
#| label: randomforest-model2-graph
#| eval: FALSE
#| echo: FALSE
#| include: FALSE
...
```{r}
#| label: randomforest-model2-diagnostics
#| eval: FALSE
#| echo: FALSE
#| include: FALSE
#| fig.height: 8
...
```{r}
#| label: randomforest-model2-pred-validation
#| echo: FALSE
#| include: TRUE
#| eval: FALSE
...
```
<!-- This creates a 2 column layout that plays nice with PDF -->
| Random Forest | Tuned Random Forest |
|:---:|:---:|
| ![](images/rf1-diag.png){width=3in} | ![](images/rf2-diag.png){width=3in} |


The balanced random forest model (RF2) showed a substantially better balance between correctly identifying defaulting and non-defaulting loans than the baseline random forest (RF1). While RF1 achieved high overall accuracy (79.9%) on the validation dataset, it correctly identified only 2.5% of loan defaults, indicating that it largely failed to detect the minority class. In contrast, RF2 achieved a lower overall accuracy of 65.7% but increased recall to 54.9%, correctly identifying more than half of all defaulting loans. This improvement came at the cost of reduced specificity (68.5% versus 99.2%) and a larger number of false positive classifications. Similar results were observed on the test dataset, where RF2 achieved a recall of 58.4% compared with only 2.8% for RF1, while maintaining comparable precision (30.9% versus 53.2%). The close agreement between validation and test performance suggests that RF2 generalized well to unseen data and was not substantially overfit to the training set. Overall, the balanced random forest represented a meaningful improvement over the baseline model by substantially increasing default detection while maintaining stable performance across datasets.


```{r}
#| label: overall-model-static-performance-table
#| echo: FALSE
...
model_results <- tribble(
  ...
  "XGBoost", "Training",   14805, 42714,  333, 14278, "40.3%", "97.7%", "25.1%", "25.7%", "39.9%",
  "XGBoost", "Validation", 12509,     0, 3134,     0, "80.0%", "0.0%",  "NA",   "100.0%", "NA",
  "XGBoost", "Test",       12545,     0, 3051,     0, "80.4%", "0.0%",  "NA",   "100.0%", "NA"
)
...
```{r}
#| label: randomforest-model-tuneRF-setup
#| echo: FALSE
#| include: FALSE
#| eval: FALSE

# https://www.statology.org/random-forest-in-r/
# Trying to optimize with tuneRF
...
```{r}
#| label: randomforest-model-tuneRF
#| echo: FALSE
#| include: FALSE
#| eval: FALSE
...
```{r}
#| label: randomforest-model-tuneRF-prediction
#| echo: FALSE
#| include: FALSE
#| eval: FALSE

 # # Commented out to allow the doc to render faster
   pred.rf.rftune = rf.rftune$predicted
   table(training[,41], pred.rf.rftune)
   # pred.rf.final
 #   #       0     1
 #   # 0 43542 13977
 #   # 1  7518  7093
 # 
   pred.rf.rftune <- predict(
   rf.rftune,
   newdata = x_validation,
   type = "class"
 )
 # 
 # 
  table(validation[,41], pred.rf.rftune)
# 
#   # pred.rf.final
#   #      0    1
#   # 0 9345 3164
...
```{r}
#| label: randomforest-model-comparison-table
#| echo: FALSE
#| warning: FALSE
#| include: false
#| eval: false
...
rf_results <- tribble(
  ...
  "tuneRF", "Training",  43542, 13977, 7518, 7093,
  "tuneRF", "Validation", 9345, 3164, 1646, 1488
) %>%
  mutate(
    Accuracy = (TP + TN) / (TP + TN + FP + FN),
...
```{r}
#| label: xgboost-model1-prediction
#| warning: FALSE
#| echo: FALSE
#| eval: FALSE
# # Get predictions
...
#| warning: FALSE
#| echo: FALSE
#| label: xgboost-model1-prediction-static-table
#| include: false
...
```{r}
#| label: xgboost-model1-diagnostics
#| warning: FALSE
#| echo: FALSE
#| eval: FALSE
...
#
# XGBoost Peformance by Threshold
...
```{r}
#| label: xgboost-model1-diagnostics-static-table
#| warning: FALSE
#| echo: FALSE
...
```{r}
#| label: test-data-rfmodel1-static-table
#| echo: false
#| include: false
...
```{r}
#| label: test-data-rfmodel2-static-table
#| warning: false
#| echo: false
#| include: false
...
```{r}
#| label: test-data-xgbmodel1-static-table
#| echo: false
#| warning: false
#| include: false
...
```{r}
#| label: overall-model-performance table
#| echo: false
#| eval: false
#| include: false
#| warning: false
...
get_metrics <- function(actual, predicted, model_name, dataset_name) {
...
# RF1 predictions
rf1_train <- rf.sim1$predicted
rf1_valid <- predict(rf.sim1, newdata = validation[drop_cols], type = "class")
rf1_test  <- predict(rf.sim1, newdata = testing[drop_cols], type = "class")


# RF2 predictions
rf2_train <- rf.sim2$predicted
rf2_valid <- predict(rf.sim2, newdata = validation[drop_cols], type = "class")
rf2_test  <- predict(rf.sim2, newdata = testing[drop_cols], type = "class")


# TuneRF predictions
tunerf_train <- rf.rftune$predicted
tunerf_valid <- predict(rf.rftune, newdata = validation[drop_cols], type = "class")
tunerf_test  <- predict(rf.rftune, newdata = testing[drop_cols], type = "class")


# XGBoost predictions
xgb_train_prob <- predict(xgb_model, dtrain)
xgb_valid_prob <- predict(xgb_model, dvalid)
xgb_test_prob  <- predict(xgb_model, dtest)

xgb_train <- ifelse(xgb_train_prob > 0.30, 1, 0)
xgb_valid <- ifelse(xgb_valid_prob > 0.30, 1, 0)
xgb_test  <- ifelse(xgb_test_prob > 0.30, 1, 0)

final_prediction_table <- bind_rows(
  
  get_metrics(y_train, rf1_train, "RF1", "Training"),
  get_metrics(y_valid, rf1_valid, "RF1", "Validation"),
  get_metrics(y_test,  rf1_test,  "RF1", "Test"),
  
  get_metrics(y_train, rf2_train, "RF2", "Training"),
  get_metrics(y_valid, rf2_valid, "RF2", "Validation"),
  get_metrics(y_test,  rf2_test,  "RF2", "Test"),
  
  get_metrics(y_train, tunerf_train, "TuneRF", "Training"),
  get_metrics(y_valid, tunerf_valid, "TuneRF", "Validation"),
  get_metrics(y_test,  tunerf_test,  "TuneRF", "Test"),
  
  get_metrics(y_train, xgb_train, "XGBoost", "Training"),
  get_metrics(y_valid, xgb_valid, "XGBoost", "Validation"),
  get_metrics(y_test,  xgb_test,  "XGBoost", "Test")
)
...
## Next Steps
There are several further steps we could take with our exploration. One potential idea is to further explore our thin file data to see if it performs any better or worse in model fitting than our normal file data.
