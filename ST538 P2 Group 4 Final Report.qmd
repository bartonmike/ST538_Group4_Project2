...
## Obstacles 
A major hurdle we encountered in the project was finding a model that fit the data well and would make accurate classifications on both default and non default data. Random forest would make accurate non-default forecasting, but would perform poorly on loans that defaulted. XGBoost at the standard threshold performed well on loans that defaulted, but not on loans that didn't. While Smote is commonly suggested as a mitigator for unbalanced data, it may not have been needed for a dataset as large as ours, and ended up only marginally decreasing our prediction errors.
