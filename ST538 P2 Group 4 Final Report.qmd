...
plot_dat <- model_results %>%
  mutate(
    Accuracy    = readr::parse_number(Accuracy),
    Recall      = readr::parse_number(Recall),
    Precision   = readr::parse_number(Precision),
    Specificity = readr::parse_number(Specificity),
    F1          = readr::parse_number(F1),
...
xgb_threshold_results %>%
  pivot_longer(
    cols = c(Recall, Precision, Specificity),
    names_to = "Metric",
    values_to = "Value"
  ) %>%
  ...
  scale_x_continuous(breaks = xgb_threshold_results$Threshold) +
  theme_minimal() +
  labs(
    title = "XGBoost Threshold Tradeoff",
